...
## 2026-06-11: #85 review retry — request_changes re-dispatches with blackboard feedback under MESH_REVIEW_RETRIES; fresh branch per attempt + prior-output merge; supersedes the #80 re-dispatch deferral

**Decision:** A `request_changes` verdict with retries left no longer fails the task: the scheduler records the reviewer's notes as a **coordinator-authored durable blackboard note** (sender-bound, so `worker.buildPrompt`'s existing primer injection carries it — no Driver/Result reshape) and leaves the task persisted `running` with no transition; the existing orphan re-dispatch path re-spawns it on the next sweep and the new diff is re-reviewed. Bounded by `MESH_REVIEW_RETRIES` (default 1, 0 = pre-#85 behavior); the counter is in-memory per coordinator lifetime, same posture as the budget meter. **Never** retries `reject` (budget conservation — feedback cannot fix a wrong approach) or any `ReviewError` (no verdict was produced; nothing to act on). A failed feedback write falls back to the typed failure — a blind retry burning a worker turn on the same diff is exactly what the design refuses. **Worker core:** the worktree allocator now treats a name as taken when its dir OR its branch exists (branches are never deleted, so a successful first run left `mesh/worker/<task>` alive and `-b` on it was a typed spawn_failed); each attempt gets a fresh numbered branch, and `mergePrior` merges the task's own recorded output branch (set by the scheduler on success) into the new worktree after `mergeDeps` and before `baseSHA` capture — the retry sees and builds on the work the feedback refers to, while its own reported diff stays incremental, exactly the dependency-inheritance posture.

**Rationale:** The #80 deferral named the two missing pieces (branch-aware re-allocation, a feedback channel into the prompt); both now exist, so the gate can spend one bounded extra worker+review turn to recover the common case (actionable notes) instead of failing the whole job. Stacking the retry on a fresh branch + self-merge (rather than re-using the surviving branch) keeps one allocator rule, preserves per-attempt evidence, handles preserved-worktree edge cases, and leaves the re-review an incremental diff of exactly the fix. The blackboard ride keeps one authority for durable cross-agent facts and zero new seams.

**Status:** active

**References:** #85, #80, #26; internal/scheduler (Options.ReviewRetries, handleReview retry case, recordFeedback), internal/worker (addWorktree branch probe, mergePrior/mergeBranch), internal/config (MESH_REVIEW_RETRIES), internal/coordinator (wiring); supersedes the "request_changes does NOT re-dispatch in this slice" sub-decision of 2026-06-10 "#80 review gating" (the rest of that entry stays in force)
