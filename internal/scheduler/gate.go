...
// queued/runnable/blocked/skipped vocabulary maps onto the five persisted
// states:
//
//	pending  → queued (waiting on deps) | runnable (all deps done) |
//	           blocked (a dep failed/cancelled; about to be skipped)
//	running  → running
//	done     → done
//	failed   → failed
...
// cancelled, or missing from the persisted DAG — a referential break that
// must never spawn work).
func blockingDep(rec task.Record, byID map[string]task.Record) (string, bool) {
	for _, dep := range rec.DependsOn {
		...
		if d.State == envelope.TaskFailed || d.State == envelope.TaskCancelled {
			return dep, true
		}
	}
	return "", false
