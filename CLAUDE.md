...
- **Review gating (#80, scheduler↔expert spine integration):** with `MESH_REVIEW_ROLE` set, every successful worker diff is routed to that role's resident expert (`KindReviewRequest` on `mesh.review-req.<role>`; the scheduler computes the diff from the shared checkout) and the task's terminal state is gated on the typed `KindReview` verdict — approve → done, request_changes/reject/error/timeout → failed (never a silent pass; re-dispatch-with-feedback is a tracked follow-up). Review cost (`ReviewPayload.CostUSD`) accrues on the same `MESH_BUDGET_USD` meter. Unset = pre-#80 behavior (success → done, no review).
...
- **Triage plan-step (external planning CLI):** with `MESH_PLAN_CLI` set, `Triager.plan` runs that CLI *before* decomposition as `<cli> <ticketfile.md> --repo <path> [--model M]` (ticket title+body written to a temp file; `MESH_PLAN_MODEL` default `sonnet`, `MESH_PLAN_TIMEOUT` default 5m) and writes its stdout to the repo blackboard as a `KindNote`/`NoteKindContext` note (truncated to `meshapi.MaxNoteLen`, marker-suffixed) — so decomposition's `recentNotes` and every worker's `mesh context` see the plan before/while implementing. Empty `MESH_PLAN_CLI` = plan-step off (default); a timeout or empty-stdout exit classifies TRANSIENT (`TriagePlannerUnavailable`, retryable), never a silent skip. `integrations/plan-cli.sh` is a shim adapting a `PLAN_ARTIFACT=<path>`-emitting plan tool (writes its plan to a file rather than stdout) to this stdout contract; requires `PLAN_TOOL` env pointing at the underlying tool.
- **Edit-capable workers:** `MESH_WORKER_PERMISSION_MODE` (default `bypassPermissions`) sets the worker CLI's `--permission-mode` so a headless worker can actually edit files and run tooling without an interactive prompt it could never answer. Empty omits the flag (pre-existing text-only behavior).
- **Live worker transcript streaming:** `MESH_WORKER_STREAM=on` runs the worker CLI in `--output-format stream-json` and tees every event line to `$MESH_DIR/runs/<task>.jsonl` as the agent works (`internal/cliexec.invokeStreaming`), while still returning only the final result envelope so callers parse it exactly like the one-shot `json` path. The dashboard's `GET /api/runlog?task=<id>` serves that file (empty body if streaming is off or the run hasn't started) and `web/runlog.html` is the live tail viewer. Off by default — zero behavior change to the proven one-shot path.
- **Jira job source:** `job.SourceJira` (`"jira"`) joins `manual`/`github` as a recognized `job.Record.Source`; a jira job carries the issue key (e.g. `CA-1234`) in `SourceRef`.
...
- **`docs/agent-runbook.md`** — process rules for implementation agents (worktrees, frozen contracts, merge order, done-gates). Any agent working the issue board reads this first.
