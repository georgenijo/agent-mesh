...
### Claim History
A chronological log of claim lifecycle events (claim / conflict / release) with relative ages, **rendered newest-first**. It is **derived observability**: the observer keeps a bounded ring and streams it in insertion order (oldest-first) on connect — replayed on every connect so it survives a page refresh — and the client reverses it for display. The claims KV remains the one authority for who currently holds what. The panel pill counts conflicts when any are present (rose), otherwise total events.
