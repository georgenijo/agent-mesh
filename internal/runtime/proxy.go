...
func (p *Proxy) spawn(ctx context.Context, extraArgs []string) error {
	...
	// session_id-bearing `init` only AFTER the first input, never on spawn. The
	// old code only ever unblocked because the operator's SessionStart hooks
	// incidentally emitted session-bearing events first — fragile, and exactly
	// what failed when those hooks stalled (#93). StartTimeout is now a short
	// liveness grace: a child still alive past it is ready, and readLoop captures
	// the session id lazily from the first turn.
