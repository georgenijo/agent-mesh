...
## 2026-06-28: Per-agent cost+model tracking lives in Dashboard memory, not the global cost ledger

**Decision:** The global `internal/cost` ledger tracks cumulative spend by model across the whole fleet — it has no per-agent dimension and will not grow one. Per-agent cost and model are accumulated inside `Dashboard` via two in-memory maps (`agentCosts map[name→USD]`, `agentModels map[name→model]`). The dashboard populates them from `KindWorker` envelopes in `recordWorker()`. Model is sourced from the agent card in the roster on the first run for each worker and cached in `agentModels` — workers leave the roster after their task completes, so a post-completion roster lookup would miss them. The accumulated state is served as a new SSE frame `"agentcosts"` (broadcast on each worker run, also pushed on SSE connect) and via `GET /api/agent-costs`.

**Rationale:** One authority per fact: the roster is the canonical source for agent identity/model; `KindWorker` envelopes are the canonical source for per-run cost. The global ledger is an aggregate meter (budget enforcement, cost window) and should not become a per-agent registry. Accumulation in Dashboard memory is the right seam — the dashboard already subscribes to all KindWorker events and holds in-memory state for claims, notes, workers, etc. No schema changes to the cost bucket or envelope contracts were required.

**Status:** active

**References:** #98, #99; internal/dashboard/dashboard.go (agentCosts, agentModels, buildAgentCostsSnap, serveAgentCosts, recordWorker), internal/dashboard/p3_test.go (TestAgentCostsAccumulateOnWorkerRun), web/app.js (onAgentCosts, agentModel, renderAgents, renderTasks, renderCost), web/index.html (cost panel)
