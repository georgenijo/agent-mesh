...
**Rationale:** Every knob was an env var read once at process start into an immutable Config — no hot-reload, no bus/KV representation, so a settings screen had to invent a new authority under the "one authority per fact" invariant. The overlay-on-every-Start design (env > settings > default) is the only one that governs the common autostart path, not just `mesh up`. Hot knobs ride the scheduler's existing per-sweep re-read of in-memory options, made mutex-guarded (the one concurrency-sensitive change, `-race` tested); everything that gates whether a subsystem exists stays restart-class because hot-swapping subsystem existence with in-flight work is unsafe. The single shared dashboard bearer token is the v1 floor for this higher-value (spend-arming) surface; arming-confirm + audit are the extra controls. Proven by golden round-trips, revision-CAS + validation unit tests, a scheduler `-race` test (concurrent `Set*` + sweep reads), coordinator overlay-on-restart / fresh-mesh-nothing-armed tests, and a cross-process e2e (fresh mesh nothing armed → invalid write typed 400 → valid hot write persists and the effective snapshot reflects it → stale rev 409). `make test`/`test-race` green.

**Status:** active

**References:** internal/settings, internal/envelope/settings.go, internal/config/config.go (Validate + Backoff), internal/scheduler/scheduler.go (Set* + hotknobs_test.go), internal/coordinator/coordinator.go + audit.go, internal/dashboard/settings.go, web/settings.html + settings.js, test/e2e/settings_test.go

**Deferred:** expert/reviewer `--model` plumbing (`EnvExpertModel` + `runtime.Options.Model` + `--model` in `DefaultArgs`/expert launch — shown read-only "not pinnable yet" in the UI); UI-driven coordinator auto-restart (guided `mesh ops down && mesh up` only in v1); a higher-privilege per-action settings-write token; per-job/per-task model override; any live-swap of subsystem construction.

---

## 2026-06-26: Autonomous on-demand experts (#117) — the coordinator spawns a resident expert itself when a role-ask/review-request targets an un-owned role

**Decision:** With `MESH_AUTO_EXPERTS=on`, the coordinator watches the two role-owner triggers — role-addressed asks (`mesh.ask.role.<role>`) and #80 review requests (`mesh.review-req.<role>`) — and the instant one targets a role no live agent fills, it launches a resident `meshd --mode expert --role R` **itself**. This is the control-plane spawn seam the architecture always named ("the Expert pool is on-demand responder agents the coordinator spawns when no live agent owns a topic"); it stays out of the data path (Q/A payloads still travel directly between the asker's and expert's sidecars). Forks resolved: **(1) trigger site** — a dedicated bus subscription on the coordinator, NOT inside `reduce()` (which must stay a fast, ordered, non-blocking reducer) and NOT in the asker's sidecar (spawn policy is the coordinator's — the single point for per-role dedup). **(2) asker fail-fast** — `ensureResponder` is relaxed so a role-ask with no owner is allowed through (ticket created + ask published) instead of failing, gated on the same flag; a direct `--to` ask still fails (nothing to spawn). **(3) publish-before-subscribe race** — `sidecar.Start` registers before it activates its ask subscription, so the cold first ask is published before the new expert listens; solved by the coordinator **buffering the triggering envelope and re-delivering it** once the expert is a live role owner (idempotent — the ticket CAS guard rejects a duplicate accept), rather than making the expert sweep open tickets on join (keeps the change localized to the coordinator). **(4) opt-in** — `MESH_AUTO_EXPERTS=on` (default off), same posture as `MESH_PLANNER_CLI`/`MESH_WORKER_CLI`: an autostarted coordinator never spawns LLM processes unless armed, and a spawner setup failure degrades to a logged warning, never a coordinator crash. **(5) binary resolution** — meshd located via `os.Executable()` (the coordinator IS meshd), not by importing `internal/autostart`, to avoid a `coordinator→autostart→observe` import cycle.

**Rationale:** The user's core ask for the mesh — "drop issues, cheap agents spin up, the coordinator reaches through and spins up the smarter long-running agents it needs, fully autonomous." Manually pre-spawning experts defeats the value ("if you spawn the expert, we don't get the value out of the mesh"). Pointing the same watcher at review requests makes autonomous **review** experts fall out for free, which is what unlocks "workers open PRs gated on a review pass" without a human pre-starting a reviewer. Proven cross-process e2e (`TestAutoExpertSpawnsOnUnownedRoleAsk`: no auth agent exists → ask the empty `auth` role → coordinator spawns `expert-auth-*` → it answers the original ask; `TestAutoExpertOffKeepsFailFast` guards the opt-in) plus full `make ci` green.

**Status:** active

**References:** #117, #80, #27, #105 (deferred idle-reaping); internal/coordinator/experts.go, internal/coordinator/coordinator.go (Start/Stop wiring), internal/sidecar/verbs_p2.go (ensureResponder relax), internal/config/config.go (MESH_AUTO_EXPERTS), test/e2e/auto_expert_test.go. Known deferred limit: a hard coordinator crash orphans spawned experts (a restarted coordinator forgets them; they re-register and are reused) — idle reaping is #105.
...
## 2026-06-10: #80 review gating — role-addressed review-request subject; verdict event is the gate input; approve→done, everything else→failed (re-dispatch deferred); opt-in via MESH_REVIEW_ROLE; review cost on the fleet budget meter

**Decision:** The #80 scheduler→expert review integration resolves its three open forks. **Transport:** a new role-addressed subject — the scheduler-side `scheduler.BusReviewer` publishes one additive `KindReviewRequest` on `mesh.review-req.<role>` (token-distinct from `mesh.review.>` so verdict taps never see requests) carrying the diff (computed via `git diff base..head` from the shared `MESH_REPOS_DIR/<repo>` checkout — the worker branch's commits outlive its worktree) plus the #26 commit metadata parsed from the worker's machine-written `[mesh worker]` summary block; the expert's existing `KindReview` event on `mesh.review.<task>` (#27) is the reply, correlated by task id — one publish serves both the gate and the dashboard/audit taps, and the tasks KV stays the only authority (the verdict is a gate input, never a second authority). The ask/ticket path was rejected: tunnelling a structured verdict through free-text Q/Answer fields means parsing contract data out of prose. **Policy:** `approve` → done (the only reviewed path to done; a success with head==base has no diff to gate and passes on its own typed worker success); `request_changes` and `reject` → failed with the verdict in the typed reason (fail-fast then cancels dependents); every `ReviewError` code, a round-trip timeout (`MESH_REVIEW_TIMEOUT`, default 5m, synthesized as a published error verdict so the audit trail stays whole), and an unreviewable summary → failed — never a silent approve. `request_changes` does NOT re-dispatch in this slice: feedback-driven bounded retry needs worker-core support (branch-aware worktree re-allocation after a successful first run + a feedback channel into the worker prompt) that is outside #80's lane — tracked as a follow-up issue. **Opt-in:** `MESH_REVIEW_ROLE` names the reviewing role; unset = pre-#80 behavior exactly (worker success → done, no review). All diffs route to that one configured role (one resident reviewer), not to each task's own role — the subject stays role-addressed, so per-task-role routing is a config evolution, not a contract change. **Budget:** a review is an expert LLM turn; its `total_cost_usd` rides additively on `ReviewPayload.CostUSD` and accrues on the same `MESH_BUDGET_USD` meter with the same pause-on-cap discipline — a paused fleet requests no review (the task stays running for the next lifetime), and review cost crossing the cap pauses the fleet.

**Rationale:** The expert capability (#27) and the scheduler (#25/#26) each kept the integration out of scope; the missing piece is pure wiring plus policy. Reusing the #27 verdict event as the reply avoids inventing a parallel response channel and keeps one publisher per fact (the expert publishes verdicts it produced; the scheduler synthesizes-and-publishes only the verdicts the expert never saw — timeout/pre-flight failures). Failing on request_changes conserves the locked scarce resource (budget): a retry without feedback would burn a worker turn on a near-identical diff, and the worker runtime cannot today re-create a task branch after a successful run (its worktree allocator probes directories, not branches). Tasks awaiting review stay persisted `running` and in the in-memory inflight set, so sweeps neither re-dispatch nor cancel them, and a coordinator restart honestly re-runs the task — the same orphan posture the scheduler already had.

**Status:** active

**References:** #80, #27, #26, #25; internal/scheduler (review.go, reviewer.go, Options.Reviewer), internal/sidecar (ServeReviews, ReviewResult.CostUSD), internal/envelope (KindReviewRequest, SubjectReviewRequest, ReviewPayload.CostUSD, AuditReview), internal/config (MESH_REVIEW_ROLE / MESH_REVIEW_TIMEOUT), internal/coordinator (BusReviewer wiring, KindReview audit fan-out), test/e2e/review_gate_test.go; extends 2026-06-09 "#25 scheduler" and "Fleet billing posture = hard cap, no overflow"
